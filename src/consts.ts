export const SITE = {
  title: "Entity.of",
  description: "Entity.of official documentation",
  defaultLanguage: "en-us",
} as const;

export const OPEN_GRAPH = {
  image: {
    src: "https://github.com/withastro/astro/blob/main/assets/social/banner-minimal.png?raw=true",
    alt:
      "astro logo on a starry expanse of space," +
      " with a purple saturn-like planet floating in the right foreground",
  },
  twitter: "",
};

export const KNOWN_LANGUAGES = {
  English: "en",
} as const;
export const KNOWN_LANGUAGE_CODES = Object.values(KNOWN_LANGUAGES);

export const GITHUB_EDIT_URL = `https://github.com/AlexandruCalinica/entity-of-docs`;

export const COMMUNITY_INVITE_URL = `https://astro.build/chat`;

// See "Algolia" section of the README for more information.
export const ALGOLIA = {
  indexName: "entity-of-xyz",
  appId: "0JHFMRLH8K",
  apiKey: "cdf21b23e3cc5fcd1f8e226d98ac561a",
};

export type Sidebar = Record<
  (typeof KNOWN_LANGUAGE_CODES)[number],
  Record<string, { text: string; link: string }[]>
>;
export const SIDEBAR: Sidebar = {
  en: {
    "Getting Started": [
      { text: "Introduction", link: "en/introduction" },
      { text: "Install", link: "en/install" },
      { text: "First steps", link: "en/first-steps" },
      { text: "Motivation", link: "en/motivation" },
      { text: "Contribution guide", link: "en/contribute" },
    ],
    API: [
      { text: "createEntityStore", link: "en/create-entity-store" },
      { text: "@Entity", link: "en/entity" },
      { text: "@Of", link: "en/of" },
      { text: "@Producer", link: "en/producer" },
    ],
  },
};
